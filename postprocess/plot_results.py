...
PLOT_CONFIG = {
    ...
    "fontsize_legend": 14,
    "save_formats": ["png"],
    "sensor_ylim_velocity": (0.0, 1.5),
    "sensor_ylim_pressure": (-0.5, 0.5),
}
...
def _plot_sensors(model_root: Path, figures_dir: Path) -> None:
    df = pd.read_csv(model_root / "sensors" / "sensor_data.csv")
    cases = sorted(
        df["case_id"].astype(str).unique(), key=lambda x: int(x) if x.isdigit() else x
    )
    for case_id in tqdm(cases, desc="Plot sensor cases"):
        ...
        for truth_col, pred_col, field_label in [
            ...
            fig, axs = plt.subplots(
                3, 3, figsize=PLOT_CONFIG["fig_sensor"], squeeze=False
            )
            for i, sid in enumerate(sensor_ids[:9]):
                ax = axs.flatten()[i]
            ...
            for j in range(len(sensor_ids[:9]), 9):
                axs.flatten()[j].set_visible(False)
            fig.tight_layout()
            _save(fig, figures_dir, f"sensor_{field_label}_{case_id}")


def _plot_forces(model_root: Path, figures_dir: Path) -> None:
...
def _plot_rmse(model_root: Path, figures_dir: Path, cfg: Dict[str, object]) -> None:
    ...
    ax.bar(
        per_case["case_id"],
        per_case["rmse_total_mean"],
        yerr=per_case["rmse_total_std"],
        color=PLOT_CONFIG["pred_color"],
        alpha=0.85,
    )
    ax.set_xlabel("Case ID (sorted by Re)")
...
def _plot_force_bars(
    model_root: Path, figures_dir: Path, cfg: Dict[str, object]
) -> None:
    ...
    for comp in ["fx", "fy"]:
        ...
        ax.bar(
            x - width / 2,
            summary[f"{comp}_targ_mean"],
            width=width,
            yerr=summary[f"{comp}_targ_std"],
            color=PLOT_CONFIG["bar_truth_color"],
            label="Ground Truth",
        )
        ax.bar(
            x + width / 2,
            summary[f"{comp}_pred_mean"],
            width=width,
            yerr=summary[f"{comp}_pred_std"],
            color=PLOT_CONFIG["pred_color"],
            label="Prediction",
        )
        ax.set_xticks(x)
...
def run(config_path: str, output_override: Optional[str]) -> None:
    ...
    _plot_sensors(model_root, figures_dir)
    _plot_forces(model_root, figures_dir)
    _plot_rmse(model_root, figures_dir, cfg)
    _plot_force_bars(model_root, figures_dir, cfg)
